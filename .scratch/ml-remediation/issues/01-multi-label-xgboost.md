...
- [ ] `train_xgboost.py` processes the target `y` into a 2D one-hot encoded matrix.
- [ ] XGBoost is wrapped in `sklearn.multioutput.MultiOutputClassifier` during `.fit()`.
- [ ] `inference.py` correctly parses the `MultiOutputClassifier` output array and maps it back to multiple active fault strings (e.g. `["sensor_drift", "cylinder_failure"]`).
- [ ] Mock data unit tests verify the model correctly predicts multiple labels simultaneously.
